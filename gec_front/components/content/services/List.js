...
const List = (props) => {
  ...
  }, [directoriesBusinessNeeds, directoriesBusinessChallenges, router]);
      ...
      <section id="overId-01">
        <div className="services-list-page padding-b">
          <div className="container">
                ...
                >
                  <ul>
                    {directoriesBusinessNeeds?.map((item, index) => {
                      return (
                        // <ServiceSelectItem check={checkActive} title={item.title} id={item.id} idInArray={theArray.map(i=>Number(i)).includes(5) ? true:false}/>
                        <ServiceSelectItem
                          check={checkActive}
                          key={index}
                          title={item.title}
            ...
            <div className="service-list-text" dangerouslySetInnerHTML={{ __html: props?.page?.text }} />
              ...
              data?.map((item, index) => { 
                if (item.published === 1 && item.active === 1){ 
                    return (
                      <Link  href={
                        item?.slug?.slug
                      } className="service-item" key={item.id}>
